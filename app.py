...
def create_booking():
    ...
    # -----------------------------------------
    # Customer Auto Message (Msg 1 + Msg 2)
    # -----------------------------------------
    customer_msg = (
        "Your service is booked via 4U.%0A"
        "For safety & 7-day guarantee, please communicate only through this number.%0A%0A"
        f"Your service (Job ID: 4U-{bid}) is recorded.%0A"
        "If any issue arises, reply here."
    )

    customer_link = f"https://wa.me/{phone}?text={customer_msg}"

    # -----------------------------------------
    # Admin Follow-up Message
    # -----------------------------------------
    follow_msg = (
        f"New Booking Received 👇%0A"
        f"Name: {name}%0APhone: {phone}%0AService ID: {service_id}%0A"
        f"Job ID: 4U-{bid}%0AAddress: {address}"
    )
    admin_link = f"https://wa.me/{admin_wh}?text={follow_msg}" if admin_wh else ""
